...
• Great for logic that detects wether or not your LLM server is up and running, or other servers that aren't your local machine.  Just checks for a ping response, not port (yet), or active API responder (coming).
...
•	Node Title: EmAySee Random String Selector
•	Category: EmAySee_Utils
•	Description: Another node for selecting a random string from two provided string inputs. Appears to have identical functionality to EmAySee_RandomStringSelectorNode.py.
...
•	Description: Replaces placeholders (like %var1% through %var10%) in a main text string with values from up to 10 string input variables. Ideal for generating dynamic prompts.
