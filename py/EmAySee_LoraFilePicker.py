...
class EmAySee_LoraFilePicker:
    ...
    def INPUT_TYPES(s):
        ...
        # This will return a list of relative filenames (e.g., "my_lora.safetensors", 
        # "subdir/another_lora.pt"). The 'return_full_paths=True' argument is removed
    ...
    def pick_lora_file(self, lora_name):
        ...
        # folder_paths.get_folder_paths("loras") returns a list of absolute paths
        # to the directories where LORAs are stored.
        lora_folders = folder_paths.get_folder_paths("loras")

        # Iterate through each LORA folder to find the selected LORA file.
        for folder in lora_folders:
            ...
            # Also check if the lora_name itself contains subdirectories (e.g., "subdir/lora.safetensors")
            # and the folder is a base path.
            # This handles cases where the dropdown might show "subdir/lora.safetensors" directly.
            if os.path.exists(os.path.join(folder, lora_name)) and os.path.isfile(os.path.join(folder, lora_name)):
                full_lora_path = os.path.join(folder, lora_name)
                break
