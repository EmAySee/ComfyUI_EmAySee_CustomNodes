...
class EmAySee_QwenResolutionOptimizerFXD:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    def optimize(self, image, scale_multiplier, method, upscale_method):
        ...
        # THE FIX: Calculate the true aspect ratio based on the final rounded dimensions
        actual_target_aspect = target_w / target_h

        samples = image.movedim(-1, 1)

        if method == "crop":
            # Use actual_target_aspect here so the crop perfectly matches the final resize
            if input_aspect > actual_target_aspect:
                new_w = int(H * actual_target_aspect)
                offset = (W - new_w) // 2
                samples = samples[:, :, :, offset:offset + new_w]
            else:
                new_h = int(W / actual_target_aspect)
                offset = (H - new_h) // 2
...
NODE_CLASS_MAPPINGS = {
    "EmAySee_QwenResolutionOptimizerFXD": EmAySee_QwenResolutionOptimizerFXD
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "EmAySee_QwenResolutionOptimizerFXD": "EmAySee_ Qwen Resolution OptimizerFXD"
}