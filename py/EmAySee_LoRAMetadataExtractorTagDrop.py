...
# These mappings are defined here for direct node loading.

class EmAySee_LoRAMetadataExtractorTagDrop: # Prepend EmAySee_ to the class name
    """
    ...
    Includes a toggle to exclude 'ss_tag_frequency' from the output.
    """
    def __init__(self):
        pass
    ...
    CATEGORY = "EmAySee/LoRA Tools" # Updated category for consistency with your prefix
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "exclude_tag_frequency": ("BOOLEAN", {"default": False}), # New toggle input
            },
            "optional": {
    ...
    def extract_metadata(self, lora_filepath, exclude_tag_frequency, lora_upload=None):
        """
        Extracts metadata from the specified .safetensors file.

        Args:
            lora_filepath (str): The full path to the .safetensors LoRA file.
            exclude_tag_frequency (bool): If True, 'ss_tag_frequency' will be removed from the output.
        ...
        try:
            with safe_open(lora_filepath, framework="pt", device="cpu") as f:
                ...
                # Apply the toggle: remove ss_tag_frequency if exclude_tag_frequency is True
                if exclude_tag_frequency:
                    if "ss_tag_frequency" in output_data:
                        del output_data["ss_tag_frequency"]
                    # If the full config was loaded, check for ss_tag_frequency within it
                    if "_metadata_source" in output_data and output_data["_metadata_source"] == "Full Kohya_ss Config (__metadata__)":
                        if "ss_tag_frequency" in output_data:
                            del output_data["ss_tag_frequency"]
...
# Register the node
NODE_CLASS_MAPPINGS = {
    "EmAySee_LoRAMetadataExtractorTagDrop": EmAySee_LoRAMetadataExtractorTagDrop,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "EmAySee_LoRA Metadata Extractor TagDrop": "EmAySee_LoRA Metadata Extractor TagDrop",
}
